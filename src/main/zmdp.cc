...
enum CommandsEnum { CMD_SOLVE, CMD_BENCHMARK, CMD_EVALUATE };
...
void setSignalHandler(int sig, void (*handler)(int)) {
  ...
  if (-1 == sigaction(sig, &act, NULL)) {
    cerr << "ERROR: unable to set handler for signal " << sig << endl;
    exit(EXIT_FAILURE);
  }
}
...
void doSolve(const ZMDPConfig &config) {
  ...
  while (!(reachedTargetPrecision || reachedTimeout || userTerminatedG)) {
    ...
    if ((elapsed - lastPrintTime > 10) || reachedTargetPrecision ||
        reachedTimeout || userTerminatedG) {
      ValueInterval intv =
...
void doBenchmark(const ZMDPConfig &config) {
  ...
  TestDriver x;
  x.batchTestIncremental(
      config,
      /* numIterations = */ p.evaluationTrialsPerEpoch, so,
...
void solveUsage(const char *cmd0) {
  cerr
      ...
      << " solve [options] <model>\n"
         ...
         "  state to console so that you can monitor progress.  When the run "
         "ends,\n"
         "  the final policy is output to the file you specify.  There are "
         "several\n"
         "  options for how to end the run: you can specify a desired regret "
         "bound\n"
         "  for the output solution, specify a fixed timeout, or just use "
         "ctrl-C to\n"
         "  interrupt the algorithm when you are satisfied (it will output the "
         "final\n"
         "  policy before exiting).\n"
         "\n"
         "Commonly used options:\n"
         "  -f        Use fast model parser (for larger RockSample and "
         "LifeSurvey problems)\n"
         "  -o <file> Specify where to write output policy [out.policy]\n"
         "  -p <#>    Terminate when policy regret is bounded to this  "
         "precision [1e-3]\n"
         "  -s <alg>  Specify search strategy, like 'frtdp', 'hsvi', or others "
         "[frtdp]\n"
         "  -t <#>    Terminate after this number of seconds wallclock time "
         "[none]\n"
         "  For many more options and more detailed descriptions, see the "
         "config file.\n"
         "\n"
         "Examples:\n"
         "  "
      << cmd0
      << " solve RockSample_4_4.pomdp\n"
         "  "
      << cmd0
      << " solve large-b.racetrack\n"
         "  "
      << cmd0
      << " solve -t 60 -o my.policy RockSample_4_4.pomdp\n"
         "  "
      << cmd0
      << " solve -s lrtdp RockSample_4_4.pomdp\n"
         "  "
      << cmd0
      << " solve -f RockSample_5_7.pomdp\n"
         "\n";
  exit(-1);
}
...
void benchmarkUsage(const char *cmd0) {
  cerr << "usage: " << cmd0
       << " benchmark [options] <model>\n"
          "  Run 'zmdp -h' for an overview of commands and generic options.\n"
          "\n"
          "  'zmdp benchmark' tests performance for a search strategy and "
          "problem\n"
          "  you select.  As the solution algorithm progresses, various "
          "statistics\n"
          "  are logged, such as the algorithm's bounds on the value of the "
          "initial\n"
          "  state, the number of backups so far, and periodic measurements of "
          "the\n"
          "  solution quality in simulation.  The plotting tools in the "
          "src/tools\n"
          ...
          "  -f        Use fast model parser (for larger RockSample and "
          "LifeSurvey problems)\n"
          "  -i <#>    Specify number of simulation runs at each evaluation "
          "epoch [100]\n"
          "  -o <file> Specify where to write output policy [none]\n"
          "  -p <#>    Terminate when policy regret is bounded to this "
          "precision [1e-3]\n"
          "  -s <alg>  Specify search strategy, like 'frtdp', 'hsvi', or "
          "others [frtdp]\n"
          "  -t <#>    Terminate after this number of seconds wallclock time "
          "[none]\n"
          "  For many more options and more detailed descriptions, see the "
          ...
          "  "
       << cmd0
       << " bench RockSample_4_4.pomdp\n"
          "  "
       << cmd0
       << " bench large-b.racetrack\n"
          "  "
       << cmd0
       << " bench -s lrtdp RockSample_4_4.pomdp\n"
          "  "
       << cmd0
       << " bench -f RockSample_5_7.pomdp\n"
          "\n";
  exit(-1);
}
...
void evaluateUsage(const char *cmd0) {
  cerr
      << "usage: " << cmd0
      << " evaluate [options] <simulatorModel>\n"
         "  Run 'zmdp -h' for an overview of commands and generic options.\n"
         "\n"
         "  'zmdp evaluate' evaluates a policy output by 'zmdp solve' or 'zmdp "
         "benchmark'.\n"
         "  The policy is evaluated by executing a number of trials in "
         "simulation;\n"
         ...
         "  "
      << cmd0
      << " eval -f --policyInputFile my.policy ltv1.pomdp\n"
         "  "
      << cmd0
      << " eval -f --policyType lspath --customModel ltv1.lifeSurvey -i 1000 "
         "ltv1.pomdp\n"
         "  "
      << cmd0
      << " eval -f --plannerModel ltv1.pomdp --simulatorModel ltv1a.pomdp\n"
         "\n";
  exit(-1);
}
...
void genericUsage(const char *cmd0) {
  cerr
      ...
      << " <command> [options]\n"
         "\n"
         "Commands:\n"
         "  zmdp solve      Solves an MDP or POMDP, generating an output "
         "policy\n"
         "  zmdp benchmark  Like 'solve', but interleaves evaluation during "
         "the solution process\n"
         "  zmdp evaluate   Evaluates a policy output by 'solve' or "
         "'benchmark'\n"
         "\n"
         "  For more information on a command, run (for example), 'zmdp solve "
         "-h'.\n"
         "\n"
         "Generic options:\n"
         "  -h or --help           Print this help\n"
         "  --version              Print version information (CFLAGS used at "
         "compilation)\n"
         "  -c or --config <file>  Specify a config file to read options from\n"
         "  --genConfig <file>     Generate an example config file and exit\n"
         "\n"
         "  ZMDP gets configuration information from three places: (1) Default "
         "values\n"
         "  are embedded in the binary at compile time.  (2) If you specify a "
         "config file\n"
         "  using the --config option, any fields set in that file override "
         "the defaults.\n"
         "  (3) You can override individual fields in the config file from the "
         "command line.\n"
         "  For instance, if the config file specifies 'searchStrategy frtdp' "
         "you can override\n"
         "  with '--searchStrategy hsvi' at the command line.\n"
         "\n"
         "  To generate an example config file (including default values and "
         "comments describing\n"
      ...
      << " solve RockSample_4_4.pomdp\n"
         "  "
      << cmd0
      << " benchmark RockSample_4_4.pomdp\n"
         "  "
      << cmd0
      << " evaluate --policyInputFile out.policy RockSample_4_4.pomdp\n"
         "\n";
  exit(-1);
}
...
int main(int argc, char **argv) {
  ...
  for (int argi = 1; argi < argc; argi++) {
    ...
    // first pass through arguments: only process --help, --config, and
    // --genConfig. all other arguments get passed through for possible alias
    // expansion
    if (args == "-h" || args == "--help") {
      usage(argv[0], cmd1);
      ...
      if (++argi == argc) {
        fprintf(stderr,
                "ERROR: found -c option without argument (use -h for help)\n");
        exit(EXIT_FAILURE);
      }
