...
void SolverParams::inferMissingValues(void) {
  ...
  if (-1 == modelType) {
    if (endsWith(probName, ".pomdp")) {
      if (zmdpDebugLevelG >= 1) {
        printf("[params] inferred modelType='pomdp' from model filename "
               "extension\n");
      }
      modelType = T_POMDP;
    } else if (endsWith(probName, ".mdp")) {
      if (zmdpDebugLevelG >= 1) {
        printf("[params] inferred modelType='mdp' from model filename "
               "extension\n");
      }
      modelType = T_MDP;
    } else if (endsWith(probName, ".racetrack")) {
      if (zmdpDebugLevelG >= 1) {
        printf("[params] inferred modelType='racetrack' from model filename "
               "extension\n");
      }
  ...
  if (-1 == lowerBoundRepresentation) {
    if (T_POMDP == modelType) {
      ...
      if (zmdpDebugLevelG >= 1) {
        printf("[params] selected lowerBoundRepresentation='maxPlanes' because "
               "modelType='pomdp'\n");
      }
    } else {
      lowerBoundRepresentation = V_POINT;
      if (zmdpDebugLevelG >= 1) {
        printf("[params] selected lowerBoundRepresentation='point' because "
               "model is an MDP\n");
      }
    }
  }
  if (-1 == upperBoundRepresentation) {
    if (T_POMDP == modelType) {
      ...
      if (zmdpDebugLevelG >= 1) {
        printf("[params] selected upperBoundRepresentation='sawtooth' because "
               "modelType='pomdp'\n");
      }
    } else {
      upperBoundRepresentation = V_POINT;
      if (zmdpDebugLevelG >= 1) {
        printf("[params] selected upperBoundRepresentation='point' because "
               "model is an MDP\n");
      }
    }
  }
  ...
  if (V_MAXPLANES == lowerBoundRepresentation && T_POMDP != modelType) {
    fprintf(stderr, "ERROR: lowerBoundRepresentation='maxPlanes' requires "
                    "modelType='pomdp' (-h for help)\n");
    exit(EXIT_FAILURE);
  }
  if (V_SAWTOOTH == upperBoundRepresentation && T_POMDP != modelType) {
    fprintf(stderr, "ERROR: upperBoundRepresentation='sawtooth' requires "
                    "modelType='pomdp' (-h for help)\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (NULL != policyOutputFile && 0 == strcmp(policyOutputFile, "-")) {
    if (usingBenchmarkFrontEnd) {
      policyOutputFile = NULL;
      if (zmdpDebugLevelG >= 1) {
        printf("[params] selected policyOutputFile='none' because front end is "
               "zmdp benchmark\n");
      }
    } else {
      policyOutputFile = "out.policy";
      if (zmdpDebugLevelG >= 1) {
        printf("[params] selected policyOutputFile='%s' because front end is "
               "zmdp solve\n",
               policyOutputFile);
      }
    }
...
void constructSolverObjects(SolverObjects &obj, SolverParams &p,
  ...
  if (zmdpDebugLevelG >= 1) {
    printf("[params] with requested searchStrategy, lowerBoundRequired=%d, "
  ...
  case -1:
    ...
    if (zmdpDebugLevelG >= 1) {
      printf("[params] selected maintainLowerBound=%d because "
  ...
  if (!(p.maintainLowerBound || p.maintainUpperBound)) {
    fprintf(stderr, "ERROR: at least one bound must be maintained, lower or "
                    "upper (-h for help)\n");
    exit(EXIT_FAILURE);
  }
  ...
  case -1:
    ...
    if (zmdpDebugLevelG >= 1) {
      printf("[params] selected useUpperBoundRunTimeActionSelection=%d because "
  ...
  case 0:
    if (!p.maintainLowerBound) {
      fprintf(stderr, "ERROR: runTimeActionSelection='lower' incompatible with "
  ...
  case 1:
    if (!p.maintainUpperBound) {
      fprintf(stderr, "ERROR: runTimeActionSelection='upper' incompatible with "
