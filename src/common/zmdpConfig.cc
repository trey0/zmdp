...
static void replace_string(string &s, const string &pattern,
...
ZMDPConfig::ZMDPConfig(void) : noNewFieldsAllowed(false) {}

void ZMDPConfig::readFromStream(const std::string &_sourceName,
                                std::istream &in) {
  ...
  while (1) {
    ...
    if (0 == strncmp(buf, "alias", 5)) {
      ...
      if (NULL == bufp) {
        fprintf(
            stderr,
            "ERROR: %s:%d: syntax error, expected 'alias <field> <value>'\n",
      ...
      if (2 != sscanf(buf, "%s %s", field, val)) {
        fprintf(stderr,
                "ERROR: %s:%d: syntax error, expected '<field> <value>'\n",
                sourceName.c_str(), lnum);
        exit(EXIT_FAILURE);
      }
      setString(field, val);
...
void ZMDPConfig::readFromString(const std::string &_sourceName,
                                const char *buf) {
