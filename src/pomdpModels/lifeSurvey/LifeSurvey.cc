...
LSState::LSState(int si) { *this = tableG.getState(si).s; }

int LSState::toInt(void) const { return tableG.getStateIndex(*this); }

bool LSState::isTerminal(void) const { return (-999 == pos.x); }
...
std::string LSState::toString(void) const {
  if (isTerminal()) {
    ...
    snprintf(buf, sizeof(buf), "sx%dy%dd%du%dn%d%d%dr%s", pos.x, pos.y,
             ...
             lifeInNeighborCell[1], lifeInNeighborCell[2], rbuf);
    return buf;
  }
}
...
std::string LSObservation::toString(void) const {
  if (isNull) {
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "o%d%d%d", lifeInNeighborConfidence[0],
             lifeInNeighborConfidence[1], lifeInNeighborConfidence[2]);
    return buf;
  }
}
...
LSStateEntry &LSStateTable::getState(int si) { return states[si]; }

/**********************************************************************
 * LSMODEL FUNCTIONS
 **********************************************************************/

LSModel::LSModel(void) : targetList(NULL) {}
...
void LSModel::calculateRegionReachable(LSGrid &result, const LSGrid &g, int r) {
...
double LSModel::getReward(int oldRewardLevel, int newRewardLevel) const {
  static double rewards[] = {0, LS_REWARD_VISITED_REGION,
                             LS_REWARD_VISITED_LIFE, LS_REWARD_SAMPLED_LIFE};
...
void LSModel::getObservations(std::vector<LSObsOutcome> &result, int spi,
                              int ai) const {
  LSAction a(ai);
  LSObsOutcome outc;
  result.clear();
  ...
  if (sp.isTerminal()) {
    ...
    FOR(i, LS_NUM_OBSERVATIONS - 1) {
      ...
      for (int j = 2; j >= 0; j--) {
        ...
        outc.prob *=
            (sp.lifeInNeighborCell[j] ? mfile.obsDistributionLifePresent[obsj]
...
void LSModel::getInitialStateDistribution(std::vector<LSOutcome> &initStates) {
  ...
  FOR(i, 3) { preInit.lifeInNeighborCell[i] = 0; }
...
void LSModel::writeToFile(FILE *outFile, bool fullIdentifiers) {
  ...
  FOR_EACH(sp, initStates) { initBelief[sp->nextState] = sp->prob; }
      ...
      FOR_EACH(op, outcomes) {
        if (fullIdentifiers) {
          ...
          fprintf(outFile, "T: %-5s : %-20s : %-20s %.15lf\n",
                  a.toString().c_str(), s.toString().c_str(),
                  sp.toString().c_str(), op->prob);
        } else {
...
void LSModel::readTargetList(std::vector<LSPos> &result, const char *fname) {
