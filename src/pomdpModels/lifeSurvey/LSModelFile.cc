...
static void convertToDoubleVector(std::vector<double> &result,
                                  const std::string &s, int doNormalizeVector) {
  ...
  if (doNormalizeVector) {
    double sum = 0.0;
    FOR_EACH(vp, result) { sum += *vp; }
    FOR_EACH(vp, result) { *vp /= sum; }
  }
}

static void writeDoubleVectorToFile(FILE *outFile, const char *header,
                                    const std::vector<double> &vec) {
  fprintf(outFile, "%s ", header);
  FOR_EACH(dp, vec) { fprintf(outFile, "%lf ", *dp); }
  fprintf(outFile, "\n");
}
...
static std::string getVal(const ParamLookup &params,
  ...
  if (params.end() == i) {
    fprintf(stderr,
            "ERROR: LSModelFile: preamble does not specify a value for "
            "parameter '%s'\n",
            paramName.c_str());
    exit(EXIT_FAILURE);
  } else {
...
static std::string parseRow(const char *inRow, int isOdd,
                            const std::string &fname, int lnum) {
  int n = strlen(inRow);
  ...
  FOR(inIndex, n) {
    ...
    if (0 == ((isOdd + inIndex) % 2)) {
      ...
      if (' ' != c) {
        fprintf(stderr,
                "ERROR: %s: line %d: found an unexpected non-space character, "
                "map should have a checkerboard structure\n",
                fname.c_str(), lnum);
...
LSModelFile::LSModelFile(void) : startX(-1), startY(-1) {}
...
preambleDone:
  ...
  if ((obsDistributionLifeAbsent.size() != LS_BASE_NUM_OBSERVATIONS) ||
    ...
    fprintf(stderr,
            "ERROR: %s: obsDistributionLife{Absent,Present} vectors must have "
            "%d entries each\n",
  ...
  while (NULL != fgets(lbuf, sizeof(lbuf), modelFile)) {
    ...
    if (strlen(lbuf) <= 1) {
      if (feof(modelFile)) {
        ...
        fprintf(
            stderr,
            "ERROR: %s: line %d: found unexpected zero-length line in map\n",
            fname.c_str(), lnum);
        exit(EXIT_FAILURE);
      }
    }
  ...
  FOR(y0, rows.size()) {
    ...
    FOR(x0, rows[y0].size()) {
      ...
      if (' ' == c) {
        ...
        fprintf(stderr,
                "ERROR: %s: while parsing map, found character '%c', expected "
                "space or a-z.\n",
                fname.c_str(), c);
        exit(EXIT_FAILURE);
      }
...
void LSModelFile::writeToFile(FILE *outFile) const {
  ...
  writeDoubleVectorToFile(outFile, "obsDistributionLifeAbsent",
                          obsDistributionLifeAbsent);
  writeDoubleVectorToFile(outFile, "obsDistributionLifePresent",
                          obsDistributionLifePresent);
  fprintf(outFile, "---\n");
