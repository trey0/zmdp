...
/*
   ...
   2. otherwise:
      a. if any of the forward cells is *both* in an unsampled region
         *and* had a positive lookahead reading, the action will be a sampling
   move. otherwise the action will be a non-sampling move. b. the move direction
   is chosen according to the following heuristics (in decreasing order of
   importance): i.   prefer cells that *both* had a positive lookahead reading
...
int LSPathAndReactExec::chooseAction(void) {
  ...
  if (currentLSState.usedLookaheadInThisCell) {
    ...
    if (-1 == bestDir) {
      fprintf(stderr,
              "ERROR: LSPathAndReactExec::chooseAction: in pos (%d,%d) -- "
...
void LSPathAndReactExec::advanceToNextState(int ai, int oi) {
  ...
  if (o.isNull) {
    ...
    FOR(i, 3) { nextLSState.lifeInNeighborCell[i] = 0; }
  } else {
...
void LSPathAndReactExec::generatePath(void) {
  ...
  getBestExtension(plannedPath, plannedPathValue, initialPrefix, initialValue);
...
void LSPathAndReactExec::getBestExtension(LSPath &bestPathMatchingPrefix,
                                          double &bestPathValue, LSPath &prefix,
    ...
    if (LS_UNREACHABLE != nextValueSoFar.regionCounts[r]) {
      if (atExit || !m.regionReachable[r].getCell(entry.pos)) {
        double probLifeIsSampledInRegion =
            1 - pow(1 - f.regionPriors[r], nextValueSoFar.regionCounts[r]);
  ...
  if (prefix.size() > 5) {
    if (0 == (numPathsEvaluated % 100000)) {
      printf("... %d%d%d%d\n", prefix[1].lastMoveDirection,
             prefix[2].lastMoveDirection, prefix[3].lastMoveDirection,
