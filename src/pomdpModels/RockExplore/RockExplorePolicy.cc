...
int QMDPPolicy::chooseAction(void) { return vfn.getMaxQAction(b); }
...
double getHBar(const REBelief &b) {
  ...
  double Hb = -sum;
  double Hu = -log(1.0 / (static_cast<double>(m.getNumStates())));
  return Hb / Hu;
}
...
double REValueFunction::valueIterationSweep(void) {
  ...
  for (int s = 0; s < static_cast<int>(V.size()); s++) {
...
double REValueFunction::getValue(int s) const { return V[s]; }
...
void HeuristicPolicy::setToInitialState(void) { b = m.getInitialBelief(); }
  ...
  switch (policyType) {
    ...
    case P_ZMDP: {
      ...
      policy->initMaxPlanes("RockExplore.pomdp",
                            /* useFastParser = */ false, "out.policy", *config);
      return policy;
    }
    default:
...
const char *getPolicyName(int policyType) {
  static const char *policyNames[] = {"qmdp", "voting", "mostLikely",
                                      "dualMode", "zmdpSolve"};
