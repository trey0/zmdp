...
void StateIndex::writeToFile(const std::string &outFile) const {
  ...
  out << "# This file is an index of states (or POMDP beliefs) referenced in "
         "other log\n"
      << "# files.  Each state is given a unique id number in the line of the "
         "form\n"
      << "# 'state <id>'.  (Note that the id numbers are guaranteed to appear "
         "in order\n"
      << "# starting with 0.)  Subsequent lines after each 'state' line "
         "specify the\n"
      << "# state as a sparse vector.  Each line has the form '<index> "
         "<value>'\n"
      << "# specifying one entry of the vector, and entries that do not appear "
         "have\n"
      << "# value 0.\n";
  ...
  while (!in.eof()) {
    ...
    if (string::npos != s.find("state")) {
      ...
      if (2 != sscanf(s.c_str(), "%d %lf", &id, &val)) {
        fprintf(stderr,
                "ERROR: %s:%d: syntax error, expected '<index> <val>'\n",
                inFile.c_str(), lnum);
        exit(EXIT_FAILURE);
      }
...
void StateIndex::writeBoundValuesToFile(const std::string &outFile,
  ...
  out << "# This file is a list of bound values for states.  Each line is in "
         "the form\n"
      << "# '<id> <lower> <upper>', where <id> is the id for a state in the "
         "state index\n"
      << "# file, <lower> is the lower bound value (or -1 if the lower bound "
         "is not being\n"
      << "# maintained), and <upper> is the upper bound value (or -1 if the "
         "upper bound is\n"
      << "# not being maintained).\n";

  FOR(i, entries.size()) {
    ValueInterval intv = bounds.getValueAt(*entries[i]);
    out << i << " " << setprecision(20) << intv.l << " " << setprecision(20)
        << intv.u << endl;
  }
...
void StateIndex::writeQValuesToFile(const std::string &outFile,
  ...
  out << "# This file is a list of Q values for states.  Each line is in the "
         "form\n"
         "# '<id> <action> <lower> <upper>', where <id> is the id for a state "
         "s in the\n"
         "# state index file, <action> is the number of an action a, <lower> "
         "is the lower bound\n"
         "# on Q(s,a) (or -1 if the lower bound is not being maintained), and "
         "<upper> is\n"
         "# the upper bound on Q(s,a) (or -1 if the upper bound is not being "
         "maintained).\n";
  ...
  FOR(i, entries.size()) {
    ...
    FOR(a, numActions) {
      ...
      out << i << " " << a << " " << setprecision(20) << intv.l << " "
...
StateLog::StateLog(StateIndex *_index) : index(_index) {}
...
void StateLog::writeToFile(const std::string &outFile) const {
  ...
  out << "# This file is a log of backups.  All backups performed by the "
         "heuristic\n"
      << "# search algorithm appear in order.  The line for a backup gives "
         "the\n"
      << "# index number of the backed up state; these index numbers reference "
         "states\n"
