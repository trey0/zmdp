...
CacheMDP::CacheMDP(MDP *_problem) : MDP(*_problem), problem(_problem) {
...
CacheMDP::~CacheMDP(void) {
  ...
  FOR_EACH(elt, nodeTable) { delete (*elt); }
}

const state_vector &CacheMDP::getInitialState(void) { return initialSI; }
...
state_vector &CacheMDP::getNextState(state_vector &result,
...
const state_vector &CacheMDP::translateState(state_vector &result,
                                             const state_vector &s) {
  result = getNode(s)->s;
  return result;
}
