...
PropertyList::PropertyList(void) { inFile = NULL; }
...
void TrackMap::readFromFile(const string &mapFileName, FILE *mapFile,
                            int lnum) {
  char lbuf[1024];
  char data[1024 * 1024];
  unsigned int i = 0;
  ...
  while (NULL != fgets(lbuf, sizeof(lbuf), mapFile)) {
    ...
    if (0 == width) {
      ...
      if (width != strlen(lbuf)) {
        fprintf(stderr,
                "ERROR: %s: line %d: map line lengths don't match (delete "
                "trailing whitespace?)\n",
...
RaceTrack::getOutcomeProbVector(outcome_prob_vector &result,
  ...
  if (IS_INITIAL_STATE(s)) {
    ...
    result(RT_CRASH) = 0;
    if (useErrorIsWind) {
      FOR(i, 8) { result(RT_SLIP + i) = errorProbability / 8; }
    } else {
...
#define RT_CHECK_OUTCOME(WHICH_OUTCOME, AX, AY)                                \
  ...
  printf("getOutcomeProbVector: s=[%s] a=%d result=[%s]\n", denseRep(s).c_str(),
         a, denseRep(result).c_str());
#endif

  return result;
}

state_vector &RaceTrack::getNextState(state_vector &result,
                                      const state_vector &s, int a, int o) {
  if (IS_INITIAL_STATE(s)) {
