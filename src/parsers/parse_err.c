...
static char *Err_Strings[NBR_ERRORS] = {
    ERROR_MSSG_0,  ERROR_MSSG_1,  ERROR_MSSG_2,  ERROR_MSSG_3,  ERROR_MSSG_4,
    ERROR_MSSG_5,  ERROR_MSSG_6,  ERROR_MSSG_7,  ERROR_MSSG_8,  ERROR_MSSG_9,
    ERROR_MSSG_10, ERROR_MSSG_11, ERROR_MSSG_12, ERROR_MSSG_13, ERROR_MSSG_14,
    ERROR_MSSG_15, ERROR_MSSG_16, ERROR_MSSG_17, ERROR_MSSG_18, ERROR_MSSG_19,
    ERROR_MSSG_20, ERROR_MSSG_21, ERROR_MSSG_22, ERROR_MSSG_23, ERROR_MSSG_24,
    ERROR_MSSG_25, ERROR_MSSG_26, ERROR_MSSG_27, ERROR_MSSG_28};
...
void ERR_enter(source, lineno, errorid, modificationString)
    /*
            The routine will create a new node for the linked list of errors and
    fill it with the proper data.  It will then insert this node into the
    existing linked list of errors in line number order.  If there already
    exists one or more errors for that line number the new node will be placed
    at the end of all of those so that for a line, first generated is the first
    displayed.  Error messages with no associated line number will always
    precede all the other messages with a line number and, similiarly, on a
    first generated-first displayed basis.
            ...
            Parameter Descriptions:
               ...
               "modificationString" is a pointer to a string of characters that
    will be used to replace the character ERR_META when the error message is
    printed out. (if ERR_META is in error message)
...
const char *modificationString;
{
  ...
  if (modificationString != NULL) {
    newNode->modString =
        (char *)calloc(strlen(modificationString) + 1, sizeof(char));
...
void ERR_cleanUp() {
  ...
  while (Err_list != NULL) {
    ...
    if (temp->source != NULL) free(temp->source);
    if (temp->modString != NULL) free(temp->modString);
    free(temp);
  } /* while */
}
/***************************************************************************/
