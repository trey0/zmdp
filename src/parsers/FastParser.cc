...
void FastParser::readPomdpFromFile(CassandraModel &pomdp,
                                   const std::string &_fileName) {
...
void FastParser::readModelFromFile(CassandraModel &problem, bool expectPomdp) {
  ...
  if (!in) {
    cerr << "ERROR: couldn't open " << problem.fileName
...
void FastParser::readModelFromStream(CassandraModel &p, std::istream &in,
                                     bool expectPomdp) {
  char buf[1 << 20];
  int lineNumber;
  char sbuf[512];
    ...
    if (inPreamble) {
      if (PM_PREFIX_MATCHES("discount:")) {
        ...
        if (0 != strcmp(sbuf, "reward")) {
          cerr << "ERROR: " << p.fileName << ": line " << lineNumber
               << ": expected 'values: reward', other types not supported by "
                  "fast parser"
               << endl;
          exit(EXIT_FAILURE);
        }
...
void FastParser::readStartVector(CassandraModel &p, char *data,
                                 bool expectPomdp) {
  if (expectPomdp) {
    // POMDP case
        ...
        if (0 == i) {
          ...
          if (startState != floor(startState)) {
            cout << "ERROR: " << p.fileName
                 << ": POMDP 'start' statement must either contain a single "
                    "integer "
