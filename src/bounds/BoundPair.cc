...
void BoundPair::initialize(MDP *_problem, const ZMDPConfig *_config) {
  ...
  if (maintainLowerBound) {
    lowerBound->initialize(BP_INITIALIZATION_PRECISION_FACTOR *
                           targetPrecision);
  }
  if (maintainUpperBound) {
    upperBound->initialize(BP_INITIALIZATION_PRECISION_FACTOR *
                           targetPrecision);
  }
...
MDPNode *BoundPair::getNode(const state_vector &s) {
  ...
  if (lookup->end() == pr) {
    ...
    FOR_EACH(hstructP, getNodeHandlers) { (*hstructP->h)(cn, hstructP->hdata); }
...
void BoundPair::update(MDPNode &cn, int *maxUBActionP) {
  ...
  if (dualPointBounds) {
    // updateDualPointBounds is an optimized procedure that only works if both
    // lower and upper bound are point bounds
